...
def gradual_weight_update(old_state_dict: OrderedDict[str, th.Tensor], 
                         new_state_dict: OrderedDict[str, th.Tensor], 
                         alpha: float = 0.5) -> OrderedDict[str, th.Tensor]:
    """
    Gradually blends old and new model weights using momentum/exponential moving average.
    
    This implements: updated_weights = (1 - alpha) * old_weights + alpha * new_weights
    
    Args:
        old_state_dict: Current global model weights (before update)
        new_state_dict: New averaged weights from clients
        alpha: Blending factor controlling update speed
               - 0.0 = no update (keep old weights completely)
               - 1.0 = full update (standard FedAvg, replace with new weights)
               - 0.3 = gradual update (70% old, 30% new) [RECOMMENDED for DP]
               - 0.5 = equal blend (50% old, 50% new)
    
    Returns:
        Blended state dictionary with gradual updates applied
    
    Benefits:
        - Reduces variance in training (smoother learning curves)
        - Dampens noise impact (critical for differential privacy)
        - Prevents catastrophic forgetting in heterogeneous settings
        - More robust to outlier clients
    
    Trade-offs:
        - Slower convergence (needs more rounds to reach same performance)
        - Lower alpha = more stability but slower learning
        - Higher alpha = faster learning but more variance
    """
    if not old_state_dict or not new_state_dict:
        return new_state_dict if new_state_dict else old_state_dict
    
    blended = OrderedDict()
    
    for key in old_state_dict.keys():
        # Exponential moving average: blend old and new weights
        blended[key] = (1 - alpha) * old_state_dict[key] + alpha * new_state_dict[key]
    
    return blended


def scale_delta_update(delta: OrderedDict[str, th.Tensor], 
                       server_lr: float = 0.1) -> OrderedDict[str, th.Tensor]:
    """
    Scales a weight delta (update) by a server learning rate.
    
    This is an alternative to gradual_weight_update() that works directly on deltas
    rather than full weights. Useful for DP-FL where we compute noisy deltas.
    
    Args:
        delta: Weight update (new_weights - old_weights)
        server_lr: Server-side learning rate (0 to 1)
                   - 0.0 = no update
                   - 1.0 = full update
                   - 0.1 = apply 10% of the update [RECOMMENDED for DP]
    
    Returns:
        Scaled delta
    
    Usage:
        scaled_delta = scale_delta_update(noisy_delta, server_lr=0.1)
        new_global = old_global + scaled_delta
    """
    scaled = OrderedDict()
    
    for key in delta.keys():
        scaled[key] = server_lr * delta[key]
