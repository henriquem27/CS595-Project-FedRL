...
from helpers import average_state_dicts, WeightStorageCallback, save_data, gradual_weight_update
...
def run_fl_experiment(NUM_ROUNDS, CHECK_FREQ, LOCAL_STEPS, clients_per_round, task_list, experiment_name="fl_run", alpha=0.3):
    ...
    print(f"Gradual Weight Update: ENABLED (alpha={alpha})")
    if alpha == 1.0:
        print("  Note: alpha=1.0 means standard FedAvg (no gradual adjustment)")
    else:
        print(f"  Blending: {(1-alpha)*100:.0f}% old weights + {alpha*100:.0f}% new weights")
    ...
    for round_num in range(NUM_ROUNDS):
        ...
        # 3. Aggregate with Gradual Weight Update
        if active_client_state_dicts:
            print("  > Aggregating weights...")
            avg_state_dict = average_state_dicts(active_client_state_dicts)
            
            # Apply gradual weight update (momentum-based blending)
            old_global_state = global_model.policy.state_dict()
            updated_global_state = gradual_weight_update(old_global_state, avg_state_dict, alpha=alpha)
            
            global_model.policy.load_state_dict(updated_global_state)
            print(f"  > Applied gradual update (alpha={alpha})")
            
            # Optional: Save global model for this round too
            logger.save_client_weights("Global_Model", round_num, updated_global_state)
