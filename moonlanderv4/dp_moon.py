...
from helpers import DynamicLunarLander, scale_delta_update
...
def run_dp_fl_experiment(NUM_ROUNDS, CHECK_FREQ, LOCAL_STEPS, task_list, DP_SENSITIVITY, DP_EPSILON, clients_per_round, experiment_name="dp_fl_run", server_lr=0.1):
    ...
    print(f"DP Settings: Sensitivity={DP_SENSITIVITY}, Epsilon={DP_EPSILON}")
    print(f"Gradual Weight Update: ENABLED (server_lr={server_lr})")
    if server_lr == 1.0:
        print("  Note: server_lr=1.0 means full update (no gradual adjustment)")
    else:
        print(f"  Applying {server_lr*100:.0f}% of each noisy delta update")
    ...
    for round_num in range(NUM_ROUNDS):
        ...
        # Aggregation with Gradual Update (Server Learning Rate)
        if active_noisy_deltas:
            ...
            # Scale the delta by server learning rate (gradual update)
            scaled_delta = scale_delta_update(avg_noisy_delta, server_lr=server_lr)
            
            # Apply scaled noisy delta to global model
            new_global_state_dict = OrderedDict()
            for key in global_state_dict.keys():
                new_global_state_dict[key] = global_state_dict[key] + scaled_delta[key]
            
            global_model.policy.load_state_dict(new_global_state_dict)
            print(f"  > Applied gradual update (server_lr={server_lr})")
