...
# ===============================================
#  New Plot 4: Average Reward by Planet Type
# ===============================================

def plot_average_reward_by_planet_type(data, output_filename="avg_reward_by_type.png", title="FRL"):
    print("\nGenerating Average Reward by Planet Type plot...")
    ...
    # 2. Calculate average reward per type per episode (or bin)
    # Since episodes can happen at different steps, we can bin them or just plot smoothed average
    
    fig, ax = plt.subplots(figsize=(12, 7))
    colors = plt.cm.get_cmap('tab10', len(unique_types))
    
    for i, client_type in enumerate(unique_types):
        type_df = df[df['type'] == client_type].sort_values(by='steps')
        if type_df.empty: continue

        # Calculate rolling average for this type
        type_df['smoothed_reward'] = type_df['reward'].rolling(
            window=REWARD_SMOOTHING_WINDOW, min_periods=1
        ).mean()

        ax.plot(
            type_df['steps'],
            type_df['smoothed_reward'],
            label=client_type,
            color=colors(i),
            alpha=0.8
        )

    ax.set_title(f'{title}: Average Reward by Planet Type')
    ax.set_xlabel('Global Timesteps')
    ax.set_ylabel('Smoothed Reward')
    ax.legend(loc='best', title="Environment Type")
    plt.grid(True, linestyle='--', alpha=0.5)
    plt.tight_layout()
