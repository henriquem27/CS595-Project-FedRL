    ...
    for round_num in range(NUM_ROUNDS):
        ...
        for idx in selected_indices:
            ...
            for key in delta.keys():
                total_l2_norm_sq += th.sum(delta[key] ** 2)

            global_l2_norm = th.sqrt(total_l2_norm_sq).item()
            clip_factor = min(1.0, DP_SENSITIVITY / (global_l2_norm + 1e-6))
            

            #Debugging
            if round_num < 3: 
                 print(f"   [Client {task_info['label']}] L2 Norm: {global_l2_norm:.4f} | Clip: {clip_factor:.4f}")
